...
.hero {
    ...
    border-radius: 24px; padding: 2.2rem 3rem; margin-bottom: 1.5rem;
...
.section-card {
    background: #13112b; border-radius: 20px; padding: 1.5rem 1.8rem;
    border: 1px solid #2a2660; margin-bottom: 1.2rem;
}
.sec-label {
    font-size: 0.68rem; font-weight: 700; text-transform: uppercase;
    letter-spacing: 1.4px; color: #6366f1; margin-bottom: 1rem; display: block;
}
...
.svc {
    background: #1e1a3f; border-radius: 14px; padding: 1rem 1.2rem;
...
.stTextInput > div > div > input {
    ...
    padding: 0.65rem 1rem !important; background: #1e1a3f !important;
}
...
.stTextArea > div > div > textarea {
    border-radius: 12px !important; border: 1.5px solid #2a2660 !important;
    color: #e0e7ff !important; font-size: 0.93rem !important; background: #1e1a3f !important;
}
.stTextArea > div > div > textarea::placeholder { color: #6366f1 !important; }
...
def auto_search(lat, lon):
    ...
    while radius <= 20.0:
        ...
        ph.markdown(f"""
        <div style="background:#1e1a3f;border:1px solid #2a2660;border-radius:14px;
             padding:1rem 1.5rem;margin-bottom:1rem;">
            <div style="font-size:0.68rem;font-weight:700;text-transform:uppercase;
                 letter-spacing:1.4px;color:#6366f1;margin-bottom:0.5rem;">🔍 Auto-scanning</div>
            <div style="font-size:0.9rem;color:#fbbf24;font-weight:600;">
                ⏳ Searching within <b>{r_label}</b>...
            </div>
            <div style="font-size:0.75rem;color:#4f46e5;margin-top:4px;">
                Expands automatically up to 20 km
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if total > 0:
            ph.markdown(f"""
            <div style="background:#052e16;border:1px solid #166534;border-radius:14px;
                 padding:1rem 1.5rem;margin-bottom:1rem;">
                <div style="font-size:0.68rem;font-weight:700;text-transform:uppercase;
                     letter-spacing:1.4px;color:#34d399;margin-bottom:0.5rem;">✅ Services found!</div>
                <div style="font-size:0.9rem;color:#34d399;font-weight:600;">
                    Found <b>{total} services</b> within <b>{r_label}</b>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    ph.markdown("""
    <div style="background:#450a0a;border:1px solid #991b1b;border-radius:14px;
         padding:1rem 1.5rem;margin-bottom:1rem;">
        <div style="font-size:0.9rem;color:#f87171;font-weight:600;">
            ⚠️ No services found within 20 km. Please call 112 directly.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown('<span class="sec-label">📍 Step 1 — Detect Location & Auto-Search Services</span>',
            unsafe_allow_html=True)
...
#status {
    ...
    border: 1px solid #2a2660; white-space: pre-line;
}
</style>
</head>
<body>
...
function detect() {
    ...
    st.innerText = 'Requesting GPS permission...\nIf a popup appears in your browser, click Allow.';
...
""", height=160)
    ...
    with col1:
        if st.button("✅ Use Location & Auto-Search"):
            ...
            if not val:
                st.warning("⚠️ Detect GPS first or type a city name")
            elif "," in val:
                try:
                    p = val.split(",")
                    ...
                    if 6.5 <= lat <= 37.5 and 68.0 <= lon <= 97.5:
                        ...
                        st.error("⚠️ Coordinates outside India")
                except:
                    st.error("⚠️ Invalid format. Try: 16.514445, 80.679274")
            else:
                with st.spinner("Finding location..."):
                    geo = Nominatim(user_agent="roadsos_v9")
                ...
                if loc and 6.5 <= loc.latitude <= 37.5:
                    ...
                    st.error("⚠️ Not found in India. Try a different city name.")
...
with right:
    st.markdown("""
    <div style="background:#1e1a3f;border-radius:16px;padding:1.2rem;
         border:1px solid #2a2660;height:100%;">
        <div style="font-size:0.68rem;font-weight:700;text-transform:uppercase;
             letter-spacing:1.4px;color:#6366f1;margin-bottom:0.8rem;">☎️ Tap to Call</div>
        <a href="tel:108" style="display:flex;justify-content:space-between;align-items:center;
           background:#13112b;border:1px solid #2a2660;border-radius:10px;
           padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🚑 Ambulance</span>
            <b style="color:#818cf8;font-size:1.05rem;">108</b>
        </a>
        <a href="tel:100" style="display:flex;justify-content:space-between;align-items:center;
           background:#13112b;border:1px solid #2a2660;border-radius:10px;
           padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🚔 Police</span>
            <b style="color:#818cf8;font-size:1.05rem;">100</b>
        </a>
        <a href="tel:101" style="display:flex;justify-content:space-between;align-items:center;
           background:#13112b;border:1px solid #2a2660;border-radius:10px;
           padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🔥 Fire</span>
            <b style="color:#818cf8;font-size:1.05rem;">101</b>
        </a>
        <a href="tel:1033" style="display:flex;justify-content:space-between;align-items:center;
           background:#13112b;border:1px solid #2a2660;border-radius:10px;
           padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🛣️ Highway</span>
            <b style="color:#818cf8;font-size:1.05rem;">1033</b>
        </a>
        <a href="tel:112" style="display:flex;justify-content:space-between;align-items:center;
           background:#4f46e5;border:1px solid #6366f1;border-radius:10px;
           padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:white;font-size:0.9rem;font-weight:700;">🆘 National SOS</span>
            <b style="color:white;font-size:1.05rem;">112</b>
        </a>
    </div>
    """, unsafe_allow_html=True)
    ...
    if nearest:
        ...
        <div class="sos-card">
            <div>
                ...
                <div class="sos-dist">
                    📏 {nearest['distance_km']} km away &nbsp;·&nbsp; {phone_txt}
                </div>
            </div>
    ...
    if st.session_state.user_location and st.session_state.services:
        ...
        if total == 0:
            ...
            <div class="empty">
                ...
                <div style="font-weight:600;color:#e0e7ff;margin:0.8rem 0 0.3rem;">
                    No services found within 20 km
                </div>
            ...
            st.markdown(
                f'<div class="radius-badge">🔍 Services within <b>&nbsp;{r_label}&nbsp;</b> of your location</div>',
                unsafe_allow_html=True)
        ...
        <div class="empty">
            ...
            <div style="font-weight:600;font-size:1rem;color:#e0e7ff;margin:1rem 0 0.4rem;">
                Click GPS → Use Location & Auto-Search
            </div>
            <div style="color:#6366f1;font-size:0.88rem;">
                Searches from 500 m and expands until services are found
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        if st.button("⚡  Get AI Emergency Guidance"):
            if situation.strip():
                ...
                st.markdown('<span class="sec-label" style="margin-top:1rem;">AI Guidance</span>',
                            unsafe_allow_html=True)
...
with tab3:
    ...
    if prompt := st.chat_input("Ask your emergency question..."):
        ...
        with st.spinner(""):
            reply = chat_with_ai(
                st.session_state.chat_history[:-1], prompt, loc_info)
