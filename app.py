...
.call-svc { display: inline-block; background: #052e16; color: #34d399 !important; font-size: 0.75rem; font-weight: 700; padding: 2px 12px; border-radius: 20px; margin-top: 5px; margin-left: 6px; text-decoration: none !important; border: 1px solid #166834; }
...
for key, default in [
    ...
    ("gps_lat", None), ("gps_lon", None), ("gps_ready", False)
]:
...
with left:
    # Key fix: GPS widget posts coords via query params
    gps_widget = st.components.v1.html("""
<!DOCTYPE html>
<html>
<head>
<style>
...
#btn:disabled { background:#059669; cursor:default; }
#result {
    padding:12px 16px; border-radius:12px; font-size:13px;
    line-height:1.6; display:none; white-space:pre-line;
    background:#1e1a3f; color:#a5b4fc; border:1px solid #2a2660;
}
#copy-row { display:none; gap:8px; align-items:center; margin-top:4px; }
#coords-display {
    flex:1; padding:10px 14px; border-radius:10px; font-size:13px;
    font-weight:600; background:#052e16; color:#34d399;
    border:1px solid #166534; font-family:monospace;
}
#copy-btn {
    padding:10px 16px; border-radius:10px; font-size:13px; font-weight:700;
    background:#4f46e5; color:white; border:none; cursor:pointer; white-space:nowrap;
}
#copy-btn:hover { background:#4338ca; }
#instr {
    padding:10px 14px; border-radius:10px; font-size:12px;
    background:#1e1a3f; color:#818cf8; border:1px solid #2a2660;
...
function detect() {
    ...
    res.style.background = '#1e1a3f';
    res.style.color = '#fbbf24';
    res.style.borderColor = '#2a2660';
    res.innerText = '⏳ Requesting GPS permission...\nIf browser asks, tap Allow.';
    btn.disabled = true;
    ...
    if (!navigator.geolocation) {
        ...
        res.style.borderColor = '#991b1b';
        res.innerText = '❌ GPS not supported.\nType your city in the box below.';
        btn.disabled = false;
        btn.innerText = '📍 Detect My Exact GPS Location';
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        function(pos) {
            ...
            res.style.background = '#052e16';
            res.style.color = '#34d399';
            res.style.borderColor = '#166534';
            res.innerText = '✅ GPS detected! Accuracy: ±' + acc + ' m';

            // Show coords + copy button
            row.style.display = 'flex';
            document.getElementById('coords-display').innerText = detectedVal;

            // Show instruction
            instr.style.display = 'block';
            instr.innerText = '👇 Paste the coordinates above into the box below, then click Use Location & Auto-Search';

            btn.innerText = '✅ GPS Ready!';

            // Try auto-fill silently
            tryFill(detectedVal);
        },
        function(err) {
            res.style.background = '#450a0a';
            ...
            if (err.code === 1) {
                res.innerText = '❌ Permission denied.\n\nFix: Click 🔒 in address bar → Location → Allow → Refresh\n\nOr type your city name below.';
            } else {
                res.innerText = '❌ GPS error. Please type your city below.';
            }
        },
...
function copyCoords() {
    ...
    if (navigator.clipboard) {
        navigator.clipboard.writeText(detectedVal).then(function() {
...
function tryFill(val) {
    ...
    var t = setInterval(function() {
        ...
        for (var i = 0; i < ins.length; i++) {
            ...
            if (ph.indexOf('gps') !== -1 || ph.indexOf('vijayawada') !== -1 ||
                ph.indexOf('city') !== -1 || ph.indexOf('fills') !== -1 ||
                ph.indexOf('paste') !== -1) {
                ins[i].value = val;
...
""", height=220)
    ...
    coord_input = st.text_input("",
        placeholder="Paste GPS coordinates here — or type city: Vijayawada",
    ...
    with col1:
        if st.button("✅ Use Location & Auto-Search"):
            ...
            if not val:
                st.warning("⚠️ Detect GPS and paste coordinates, or type a city name")
            elif "," in val:
                try:
                    p = val.split(",")
                    ...
                    st.error("⚠️ Invalid. Format: 16.514445, 80.679274")
            else:
...
with right:
    st.markdown("""
    <div style="background:#1e1a3f;border-radius:16px;padding:1.2rem;border:1px solid #2a2660;height:100%;">
    ...
    if st.session_state.user_location and st.session_state.services:
        ...
        <div class="empty">
            ...
            <div style="font-weight:600;font-size:1rem;color:#e0e7ff;margin:1rem 0 0.4rem;">
                Click GPS → Copy Coordinates → Paste → Use Location
            </div>
            <div style="color:#6366f1;font-size:0.88rem;">Searches from 500 m and expands until services found</div>
        </div>
        """, unsafe_allow_html=True)
