...
.dial-row { display: grid; grid-template-columns: repeat(5,1fr); gap: 8px; margin-top: 0.8rem; }
.dial-btn {
...
.dial-btn:hover { border-color: #4f46e5; background: #2a2660; transform: translateY(-2px); }
.dial-btn .di { font-size: 1.2rem; }
.dial-btn .dn { font-size: 1rem; font-weight: 700; color: #818cf8 !important; display: block; }
.dial-btn .dl { font-size: 0.68rem; color: #6366f1; display: block; margin-top: 1px; }

.gps-status-ok { background: #052e16; border: 1px solid #166534; border-radius: 10px; padding: 0.5rem 1rem; font-size: 0.85rem; color: #34d399; margin-top: 0.5rem; }
.gps-status-err { background: #450a0a; border: 1px solid #991b1b; border-radius: 10px; padding: 0.5rem 1rem; font-size: 0.85rem; color: #f87171; margin-top: 0.5rem; }
...
def auto_search(lat, lon):
    ...
    <div style="background:#450a0a;border:1px solid #991b1b;border-radius:14px;padding:1rem 1.5rem;margin-bottom:1rem;">
        <div style="font-size:0.9rem;color:#f87171;font-weight:600;">⚠️ No services found within 20 km. Please call 112 directly.</div>
    </div>
    """, unsafe_allow_html=True)
...
with left:
    # GPS iframe — fully self-contained, fills input automatically
...
#btn:disabled { background: #059669; cursor: default; opacity: 1; }
#status {
    margin-top: 10px; padding: 10px 14px; border-radius: 10px;
    font-size: 13px; line-height: 1.5; display: none;
    background: #1e1a3f; color: #a5b4fc; border: 1px solid #2a2660;
}
</style>
</head>
<body>
...
function detect() {
    ...
    st.innerText = 'Requesting GPS permission from browser...';

    if (!navigator.geolocation) {
        st.style.background = '#450a0a';
        st.style.color = '#f87171';
        st.style.borderColor = '#991b1b';
        st.innerText = '❌ GPS not supported. Please type your city name below.';
        btn.disabled = false;
        btn.innerText = '📍 Detect My GPS Location';
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        function(pos) {
            ...
            st.innerText = '✅ GPS detected: ' + val + '\n→ Now click the USE LOCATION button below!';
            ...
            var timer = setInterval(function() {
                ...
                for (var i = 0; i < inputs.length; i++) {
                    ...
                    if (ph.indexOf('gps') !== -1 || ph.indexOf('vijayawada') !== -1 || ph.indexOf('city') !== -1 || ph.indexOf('type') !== -1) {
                        inputs[i].value = val;
                ...
                if (!filled && attempts > 30) clearInterval(timer);
            }, 300);
        },
        function(err) {
            st.style.background = '#450a0a';
            ...
            st.style.borderColor = '#991b1b';
            if (err.code === 1) {
                st.innerText = '❌ Location permission denied.\nPlease type your city name in the box below instead.';
            } else {
                st.innerText = '❌ GPS error (' + err.message + ').\nPlease type your city name below.';
            }
            btn.disabled = false;
...
""", height=130)
        ...
        if st.button("✅ Use Location & Auto-Search"):
            ...
            if not val:
                ...
                if loc and 6.5 <= loc.latitude <= 37.5:
                    ...
                    st.error("⚠️ Location not found in India. Try a different city name.")
    ...
    if st.session_state.user_location:
        ...
        <div class="loc-found">
            ...
            {"&nbsp;·&nbsp; 🔍 " + str(total) + " services found within " + r_label if r_label else ""}
        </div>
        """, unsafe_allow_html=True)

with right:
    st.markdown("""
    <div style="background:#1e1a3f;border-radius:16px;padding:1.2rem 1.2rem;border:1px solid #2a2660;height:100%;">
        ...
        <a href="tel:108" style="display:flex;justify-content:space-between;align-items:center;background:#13112b;border:1px solid #2a2660;border-radius:10px;padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🚑 Ambulance</span><b style="color:#818cf8;font-size:1.05rem;">108</b>
        </a>
        <a href="tel:100" style="display:flex;justify-content:space-between;align-items:center;background:#13112b;border:1px solid #2a2660;border-radius:10px;padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🚔 Police</span><b style="color:#818cf8;font-size:1.05rem;">100</b>
        </a>
        <a href="tel:101" style="display:flex;justify-content:space-between;align-items:center;background:#13112b;border:1px solid #2a2660;border-radius:10px;padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🔥 Fire</span><b style="color:#818cf8;font-size:1.05rem;">101</b>
        </a>
        <a href="tel:1033" style="display:flex;justify-content:space-between;align-items:center;background:#13112b;border:1px solid #2a2660;border-radius:10px;padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:#e0e7ff;font-size:0.9rem;">🛣️ Highway</span><b style="color:#818cf8;font-size:1.05rem;">1033</b>
        </a>
        <a href="tel:112" style="display:flex;justify-content:space-between;align-items:center;background:#4f46e5;border:1px solid #6366f1;border-radius:10px;padding:10px 14px;margin:5px 0;text-decoration:none;">
            <span style="color:white;font-size:0.9rem;font-weight:700;">🆘 National SOS</span><b style="color:white;font-size:1.05rem;">112</b>
        </a>
    </div>
    """, unsafe_allow_html=True)
